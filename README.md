...
Place trained ML models in `fraud_detection/` directory:
...
✅ **IP Datacenter Detection** - Two-layer approach (string match + CIDR fallback)  
✅ **Case Note Hard Limit** - 400 character maximum for consistency  
✅ **Merchant Recurrence Escalation** - Automatic flag when ≥3 CRITICAL in 24h  
✅ **Update Case Status Prerequisites** - Enforces add_case_note before status change  
✅ **Fallback Mechanisms** - Graceful handling when customer profile returns no data  
✅ **Velocity Per Merchant** - Enhanced feature engineering for merchant pattern analysis  
✅ **False Positive Feedback** - Validates feedback against known rules  

## Performance Metrics

- **Average Scoring Latency**: <200ms
- **Investigation Cost**: 0.12-0.45 (normalized units)
